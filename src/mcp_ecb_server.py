...
class ECBDataServer:
    ...
    def _parse_ecb_json_data(self, json_data: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            # Check for valid ECB JSON structure
            if not isinstance(json_data, dict):
                return {"error": "Invalid JSON data: not a dictionary"}
            
            if "dataSets" not in json_data:
                return {"error": "Invalid ECB JSON data: missing dataSets field"}
                
            if not json_data["dataSets"]:
                return {"observations": [], "count": 0}
                
            if "structure" not in json_data:
                return {"error": "Invalid ECB JSON data: missing structure field"}
    ...
    def _parse_ecb_csv_data(self, csv_text: str) -> dict[str, Any]:
        ...
        try:
            if not isinstance(csv_text, str):
                return {"error": "Invalid CSV data: not a string"}
                
            lines = csv_text.strip().split("\n")
            if len(lines) < 1:
                return {"error": "Invalid CSV data: empty content"}
            ...
            if len(header) < 2:
                return {"error": "Invalid CSV data: insufficient columns"}
